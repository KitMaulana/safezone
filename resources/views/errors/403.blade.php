...
@section('content')
    <div class="text-center">
        ...
        <span class="mx-auto mt-6 flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-slate-400">
            <x-icon.ban class="h-8 w-8" />
        </span>

        <p class="mt-4 text-5xl font-extrabold text-slate-300">403</p>
        <h1 class="mt-2 text-xl font-bold text-slate-900">Akses ditolak</h1>
        <p class="mt-2 text-sm text-slate-600">{{ $exception?->getMessage() ?: 'Anda tidak memiliki izin untuk membuka halaman ini.' }}</p>
